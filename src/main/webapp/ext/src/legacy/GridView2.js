...
Ext.extend(Ext.grid.GridView, Ext.grid.AbstractGridView, {
    ...
    cellRE: new RegExp("(?:.*?)x-grid-(?:hd|cell|csplit)-(?:[\\d]+)-([\\d]+)(?:.*?)"),

    findRE: new RegExp("\\s?(?:x-grid-hd|x-grid-col|x-grid-csplit)\\s"),
    ...
    render : function(){
        ...
        if(this.grid.enableColumnResize !== false && Ext.grid.SplitDragZone){
            _ = new Ext.grid.SplitDragZone(this.grid, this.lockedHd.dom, this.mainHd.dom);
        }

        this.updateSplitters();

        if(this.grid.enableColumnMove && Ext.grid.HeaderDragZone){
        	_ = new Ext.grid.HeaderDragZone(this.grid, this.lockedHd.dom, this.mainHd.dom);
        	_ = new Ext.grid.HeaderDropZone(this.grid, this.lockedHd.dom, this.mainHd.dom);
        }
