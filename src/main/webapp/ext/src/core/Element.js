            ...
            function(prop){
                ...
                if(prop == 'opacity'){
                    if(typeof el.style.filter == 'string'){
                        var m = el.style.filter.match(new RegExp("alpha\\(opacity=(.*)\\)","i"));
                        if(m){
...
el.un('click', this.handlerFn);
     ...
     */
     setOpacity : function(opacity, animate){
        if(!animate || !A){
            ...
            if(Ext.isIE){
                s.zoom = 1;
                s.filter = (s.filter || '').replace(new RegExp("alpha\\([^\\)]*\\)","gi"),"") +
    ...
    * @return {Ext.Element} this
    ...
    getAlignToXY : function(el, p, o){
        ...
        p = p.toLowerCase();
        var m = p.match(new RegExp("^([a-z]+)-([a-z]+)(\\?)?$"));
        if(!m){
...
el.alignTo("other-el", "c-bl", [-6, 0]);
    ...
    clearOpacity : function(){
        if (window.ActiveXObject) {
            if(typeof this.dom.style.filter == 'string' && 
            		(new RegExp("alpha","i")).test(this.dom.style.filter)){
    ...
    * @return {Ext.Element} this
    ...
    update : function(html, loadScripts, callback){
        ...
        E.onAvailable(id, function(){
            ...
            var srcRe = new RegExp("\\ssrc=([\\'\"])(.*?)\\1","i");
            var typeRe = new RegExp("\\stype=([\\'\"])(.*?)\\1","i");
        ...
        dom.innerHTML = html.replace(new RegExp("(?:<script.*?>)((\\n|\\r|.)*?)(?:<\\/script>)","ig"), "");
        return this;
    },
    ...
    */
    ...
    wrap: function(config, returnDom){
        ...
        _= newEl.dom ? newEl.dom.appendChild(this.dom) : newEl.appendChild(this.dom);
        return newEl;
    },
...
// private
El.unitPattern = new RegExp("\\d+(px|em|%|en|ex|pt|in|cm|mm|pc)$","i");
