...
Ext.XTemplate = function(){
    ...
    var re = new RegExp("<tpl\\b[^>]*>((?:(?=([^<]+))\\2|<(?!tpl\\b[^>]*>))*?)<\\/tpl>");

    var nameRe = new RegExp("^<tpl\\b[^>]*?for=\"(.*?)\"");
    var ifRe = new RegExp("^<tpl\\b[^>]*?if=\"(.*?)\"");
    var execRe = new RegExp("^<tpl\\b[^>]*?exec=\"(.*?)\"");
    var m, id = 0;
    var tpls = [];
    ...
    compileTpl : function(tpl){
        ...
        if(Ext.isGecko){
            body = "tpl.compiled = function(values, parent, xindex, xcount){ return '" +
                   tpl.body.replace(new RegExp("(\\r\\n|\\n)","g"), '\\n').replace(new RegExp("'","g"), "\\'").replace(this.re, fn).replace(this.codeRe, codeFn) +
                    "';};";
        }else{
            body = ["tpl.compiled = function(values, parent, xindex, xcount){ return ['"];
            body.push(tpl.body.replace(new RegExp("(\\r\\n|\\n)","g"), '\\n').replace(new RegExp("'","g"), "\\'").replace(this.re, fn).replace(this.codeRe, codeFn));
            body.push("'].join('');};");
            body = body.join('');
        }
        eval(body);
