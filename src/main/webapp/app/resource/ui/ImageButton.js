		...
		.extend(
				...
				Ext.Button,
				{
					onRender : function(ct, position) {
						this.disabledImgPath = this.disabledImgPath
								|| this.imgPath;
						var tplHTML = '<div style="margin:0px 0px 0px 0px; border:0px; color: white; font-weight:normal" ><img src="{imgPath}" border="0" width="{imgWidth}" height="{imgHeight}" alt="{tooltip}" style="cursor: {cursor};"/> <br>{imgText:htmlEncode}</div>';
						var tpl = new Ext.Template(tplHTML);
						var btn, targs = {
							imgPath : this.disabled ? this.disabledImgPath
									: this.imgPath,
