...
app.view.notice.NoticeEditWindow = Ext
		.extend(
				Ext.Window,
				{
					title : '编辑通知公告',
					...
					initComponent : function() {
						...
						var importantStore = new Ext.data.SimpleStore({
							fields : [ 'id', 'name' ],
							data : [ [ '普通', '普通' ], [ '重要', '重要' ],
									[ '紧急', '紧急' ] ]
						});
						...
						app.view.notice.NoticeEditWindow.superclass.initComponent
					...
					save : function(button) {
						...
						if (deptIds == '') {
							Ext.MessageBox.confirm('提示',
									'您没有选择机构，系统将默认下发给所有培训机', function(btn) {
										if (btn == 'yes') {
											win
													.saveNotice(win, deptIds,
															docIds);
										}
									});
						} else {
							win.saveNotice(win, deptIds, docIds);
						}
					},
					upFirst : function() {// 置顶
						var me = this;
					...
					saveNotice : function(win, deptIds, docIds) {
					...
					saveTask : function(win, deptIds, docIds) {
						...
						form
								...
								.submit(
										{
											url : 'cn/wizool/bank/servlet/NoticeServlet?method=create',
											method : 'POST',
											timeout : 300,
											params : {
												docIds : docIds,
